...
from app.contracts.job_post import JobPostExtractionRequest, JobPostGenerationRequest
from app.contracts.llm import StructuredResponse
from app.core.security import (
    InternalPrincipal,
    require_job_post_extraction_principal,
    require_job_post_generation_principal,
)
...
@router.post("/generate", response_model=StructuredResponse, response_model_by_alias=True)
async def generate_job_post(
    request: JobPostGenerationRequest,
    _: InternalPrincipal = Depends(require_job_post_generation_principal),
    provider: LlmProvider = Depends(get_llm_provider),
) -> StructuredResponse:
    """Generate a structured JD draft from recruiter facts supplied by the backend."""
